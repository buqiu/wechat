...
class Message extends WeChat
{
    ...
    //取消关注事件
    const EVENT_TYPE_UNSUBSCRIBE = 'subscribe';
    //未关注用户扫描二维码事件
    ...
    {
        $url = $this->apiUrl . '/cgi-bin/message/mass/sendall?access_token=' . $this->getAssessToken();
    ...
    {
        $url = $this->apiUrl . '/cgi-bin/message/mass/preview?access_token=' . $this->getAssessToken();
        $content = $this->curl($url, json_encode($data, JSON_UNESCAPED_UNICODE));
        return $this->get($content);
    }
